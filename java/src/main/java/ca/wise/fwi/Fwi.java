...
//			}
	...
	/**
	 * Calculates hourly FFMC (fine fuel moisture code), using the experimental
	 * Equilibrium mathematical model developed by K. Anderson, based on the
	 * previous hour's FFMC and provided conditions. FFMC is a numerical rating
	 * of the moisture content of litter and other cured fine fuels.
	 *
	 * @param inFFMC
	 *            The previous time's Van Wagner FFMC value.
	 * @param rain
	 ...
	 * @return Calculated FFMC value. (-98.0 on failure)
	 */
	public static double hourlyFFMCEquilibrium(double inFFMC, double rain,
			double temperature, double rh, double ws, long span) {
		if ((inFFMC < 0.0) || (inFFMC > 101.0) || (temperature > 60.0)
				|| (rain < 0.0) || (rain > 300.0) || (span > 7200)
				|| (span <= 0))
		...
		double hourFrac = (double) span / 3600.0;
		double hour_frac2 = hourFrac - Math.floor(hourFrac);
		...
		double mo = factor * (101.0 - inFFMC) / (59.5 + inFFMC); // Equation 2a
		if (rain > 0.0) {
			mo += rain * 42.5 * (Math.expm1(-100.0 / (251.0 - mo)) + 1.0)
					* (-Math.expm1(-6.93 / rain)); // Equation 12
			if (mo > 150.0)
				mo += Math.pow(0.0015 * (mo - 150.0), 2.0) * Math.sqrt(rain); // Equation
																				// 13
			if (mo > 250.0)
				mo = 250.0;
		}

		double ed = 0.942 * Math.pow(rhp, 0.679) + 11.0
				* (Math.expm1((rhp - 100.0) / 10.0) + 1.0) + 0.18
				* (21.1 - temperature) * (-Math.expm1(-0.115 * rhp)); // Equation
																		// 8a
		double ew = 0.618 * Math.pow(rhp, 0.753) + 10.0
				* (Math.expm1((rhp - 100.0) / 10.0) + 1.0) + 0.18
				* (12.1 - temperature) * (-Math.expm1(-0.115 * rhp)); // Equation
																		// 8b
		...
		double cf = 59.5 * (250.0 - m) / (factor + m);
	...
	/**
	 * Calculates the previous hourly FFMC (fine fuel moisture code), using
	 * equilibrium mathematical model, based on the current hour's FFMC and
	 * previous hour's provided conditions. FFMC is a numerical rating of the
	 * moisture content of litter and other cured fine fuels.
	 *
	 * @param currentFFMC
	 ...
	 * @return Calculated FFMC value. (-98.0 on failure)
	 */
	public static double hourlyFFMCEquilibriumPrevious(double currentFFMC,
			double rain, double temperature, double rh, double ws) {
		if ((currentFFMC < 0.0) || (currentFFMC > 101.0)
				|| (temperature > 60.0) || (rain < 0.0) || (rain > 300.0))
			return -98;
		...
		outFFMC = hourlyFFMCEquilibrium(inFFMC, rain, temperature, rh, ws, hour);

		diffMC = Math.abs(outFFMC - currentFFMC);
		while (diffMC > TOLERANCE) {
			if (outFFMC > currentFFMC)
				inFFMC -= diffMC / 2.0;
			else
				inFFMC += diffMC / 2.0;

			outFFMCPrior = outFFMC;
			outFFMC = hourlyFFMCEquilibrium(inFFMC, rain, temperature, rh, ws,
					hour);
			// check for error conditions
			if (outFFMC < 0.0 || outFFMC > 101.0) {
				diffMC = 0.0;
	 ...
	 * @param ws
	 * @param seconds_into_day
	 * @return (-98.0 on failure)
	 */
	public static double hourlyFFMCHybrid(double prevFFMC, double currFFMC,
			double inFFMC, double[] rain48, double temperature, double rh,
			double ws, long seconds_into_day) {
		if (seconds_into_day > 86400)
			return -98;
		if ((prevFFMC < 0.0) || (prevFFMC > 101.0) || (currFFMC < 0.0)
				|| (currFFMC > 101.0) || (temperature > 60.0)
				|| (seconds_into_day < -43200) || (seconds_into_day >= 126000))
			return -98;
		...
		double rain = rain48[0];
		double rain24 = 0.0;
		short i;

		// if we got rain in the last hour, then we use Van Wagner
		if (rain >= 0.5)
			return hourlyFFMCVanWagner(inFFMC, rain, temperature, rh * 0.01,
					ws, 3600);

		// add up the rain from the past 24 hours
		for (i = 0; i < 24; i++)
			if (rain48[i] < 0)
				break;
			else
		...
		// if we got enough rain in the last 24 hours, then we also use Van
		// Wagner
		if (rain24 >= 0.5)
			return hourlyFFMCVanWagner(inFFMC, rain, temperature, rh * 0.01,
					ws, 3600);

		// 2nd half of the day and no rain in the past 24 hours so calculate
		// Lawson
		if (seconds_into_day >= 43200)
			return calcHourlyFFMCLawson(currFFMC, seconds_into_day, rh);

		// we had all data for the prior 24 hours, so we can try to go back even
		// further
		if (i == 24) {
			long hour = 36 + ((long) (((double) seconds_into_day) / 3600.0))
					- (((long) (((double) seconds_into_day) / 86400.0)) * 24);
			for (; i < hour; i++) {
				if (rain48[i] < 0)
					break;
				else {
		...
		return calcHourlyFFMCLawson(prevFFMC, seconds_into_day, rh);
	}
